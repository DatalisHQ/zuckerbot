...
export function registerTools(server: McpServer, client: ZuckerBotClient): void {
  ...
  server.tool(
    ...
    {
      campaign_id: z.string().describe("ZuckerBot draft campaign ID from zuckerbot_create_campaign; NOT a Meta ID from the spec builder (use zuckerbot_resume_campaign for those)"),
  ...
  // Explicit, reviewed activation of existing Meta objects (including spec builds).
  server.tool(
    ...
    {
      business_id: z.string().uuid().optional().describe("Business owning the selected Meta ad account"),
      campaign_id: z.string().regex(/^\d{1,40}$/).describe("Real numeric Meta campaign ID returned by the spec builder or insights; not a ZuckerBot draft ID"),
      entity_level: z.enum(["campaign", "adset", "ad"]).default("campaign"),
      entity_id: z.string().regex(/^\d{1,40}$/).optional().describe("Meta ad set or ad ID; required for those entity levels"),
      dry_run: z.boolean().default(true),
      confirm_spend: z.boolean().default(false).describe("True only after the user approves activation and the previewed spend controls"),
      preview_hash: z.string().regex(/^[a-f0-9]{64}$/).optional().describe("Hash from a fresh dry-run for this exact object; required to execute"),
      idempotency_key: z.string().regex(/^[A-Za-z0-9][A-Za-z0-9_.:-]{7,127}$/).optional().describe("Required to execute; reuse the exact request and key on retries"),
    },
    async (args) => {
      try {
        return formatResult(await client.post("/meta/resume", args));
  ...
  server.tool(
    ...
    {
      business_id: z.string().uuid().optional().describe("Business owning a spec-built campaign; use when several businesses are connected"),
      campaign_id: z.string().optional().describe("ZuckerBot campaign ID or real Meta campaign ID (required when entity_level is campaign)"),
      entity_level: z
    ...
    async ({ business_id, campaign_id, entity_level, entity_id, action }) => {
      try {
  ...
  server.tool(
    ...
    async ({ select_id }) => {
      try {
        ...
        const pageCount = asRecord(result)?.page_count;
        if (pageCount === 0) {
          return formatResult(appendHint(result, "ZERO pages are visible to this Facebook connection — do NOT retry in a loop; listing again will not help. Follow no_pages_hint: the human must reconnect Facebook keeping pages_show_list + business_management granted and the specific Page ticked (Meta remembers previous declines), and if the Page is portfolio-owned their profile needs a role on the Page itself. Linking assets under Datasets & Pixels does not grant page visibility."));
        }
